...
tab_abs, tab_rel = st.tabs(["Absolute Amounts", "Baker's Percentage"])
with tab_abs:
    st.text("Input your absolute amounts!")
    with st.form("abs"):
        w, s = st.columns(2)
        a_sd, a_wh, a_fw = st.columns(3)
        a_sd.number_input("Sourdough [g]", min_value=25, max_value=500,
                                    help="Sourdough in grams. Assuming 100% hydration.",
                                    key="abs_sourdough")
        a_wh.number_input("Wheat [g]", min_value=0, max_value=2000,
                                help="Wheat in grams.", key="abs_wheat")
        a_fw.number_input("Full Wheat [g]", min_value=0, max_value=2000,
                                        help="Full Wheat in grams.", key="abs_full_wheat")
        w.number_input("Water [g]", min_value=100, max_value=4000,
                               help="Water in grams.", key="abs_water")
        s.number_input("Salt [g]", min_value=1, max_value=50,
                              help="Salt in grams.", key="abs_salt")
        st.form_submit_button("Get Bread! (Abs)", on_click=get_recipe, args=["abs"])
        
    
with tab_rel:
    st.text("Input your Baker's Percentage")
    with st.form("rel"):
        w_bp, s_bp = st.columns(2)
        sd, wh, fw = st.columns(3)
        sd.number_input("Sourdough [g]", min_value=25, max_value=500,
                                    help="Sourdough in grams. Assuming 100% hydration.",
                                    key="rel_sourdough")
        wh.number_input("Wheat [g]", min_value=0, max_value=2000,
                                help="Wheat in grams.", key="rel_wheat")
        fw.number_input("Full Wheat [g]", min_value=0, max_value=2000,
                                        help="Full Wheat in grams.", key="rel_full_wheat")
        w_bp.slider("Water BP", min_value=1.5, max_value=2., step=.01,
                               help="Water in Baker's Percentage.", key="rel_water")
        s_bp.slider("Salt BP", min_value=1., max_value=1.05, step=0.005, format="%f",
                              help="Salt in Baker's Percentage.", key="rel_salt")
...
if st.session_state.bread is not None:
    ...
    st.markdown(f"""
                ...
                - Salt: __{recipe.salt_bp:.1f} BP__
                - Water: __{recipe.water_bp:.1f} BP__
